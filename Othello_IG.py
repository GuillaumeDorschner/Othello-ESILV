...
def load_image(image_name):
    """Loads an image from the assets directory using an absolute path.

    Args:
      image_name (str): The name of the image file to load.

    Returns:
      PhotoImage: The loaded image.
    """
    # Gets the directory of the current script
    script_dir = os.path.dirname(__file__)

    # Constructs the absolute path to the image
...
class Interface:
    ...
    def __init__(self, fenetre):
        ...
        self.button1 = Button(
            self.fenetre, text="Joueur contre Joueur", command=lambda: self.Joueur(2)
        )
        self.button2 = Button(
            self.fenetre, text="Joueur contre Ordinateur", command=lambda: self.Ordi()
        )
    ...
    def Ordi(self):
        ...
        self.buttonBotCommence = Button(
            self.fenetre, text="Joueur", command=lambda: self.Joueur(1, 1)
        )
        self.buttonPlayerCommence = Button(
            self.fenetre, text="IA", command=lambda: self.Joueur(1, -1)
        )
    ...
    def Actualiser_plateau(self, appeler=False):
        ...
        restart = Button(
            self.fenetre, text="Recommencer", command=lambda: self.Rejouer()
        )
        restart.place(x=600, y=130)
        rond_noir = load_image("rond_noir.png")
        rond_blanc_last = load_image("rond_blanc_last.png")
        rond_noir_last = load_image("rond_noir_last.png")
        rond_blanc = load_image("rond_blanc.png")
        possible_pion = load_image("rond_circle.png")
        vide = load_image("empty.png")
                ...
                if self.jeu.plateau[i][j] == 1:
                    if i == self.last_x and j == self.last_y:
                        self.buttons[i][j].configure(
                            image=rond_blanc_last, command=self.Pion_Pres
                        )
                    else:
                        self.buttons[i][j].configure(
                            image=rond_blanc, command=self.Pion_Pres
                        )
                elif self.jeu.plateau[i][j] == -1:
                    if i == self.last_x and j == self.last_y:
                        self.buttons[i][j].configure(
                            image=rond_noir_last, command=self.Pion_Pres
                        )
                    else:
                        self.buttons[i][j].configure(
                            image=rond_noir, command=self.Pion_Pres
                        )
    ...
    def Tour_IA(self):
        ...
        temps = 0
        manger = self.jeu.Pose()
        coord = [[i, j] for i in range(8) for j in range(8) if len(manger[i][j]) != 0]
        if len(coord) > 0:
        ...
        if not (best_y is None or best_x is None):
            self.labelCoup = Label(
                self.fenetre, text=f"Le coup de l'IA \nx: {best_x + 1} y: {best_y + 1}"
            )
        ...
        self.labelTempsTotal = Label(
            self.fenetre, text=f"Temps Total : {self.tempsTotal:.2f} s"
        )
